...
For nomad lions, however, roaming would be utilizing random movement to find better positions (solutions). These lions can only go as far as a set range of movement per iteration that is randomly chosen with uniform distrbution. Nomad lions, being independent from others, may get stuck in local optima. Getting stuck in local optima, the nomad lion won't do any useful searching when it happens. Thus, a reset should be done whenever this happens.
